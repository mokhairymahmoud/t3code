...
export const makeKiroTextGeneration = Effect.fn("makeKiroTextGeneration")(function* (
  ...
  const runKiroJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      // Kiro binds its model on the command line, so the selection is
      // applied at spawn rather than through `session/set_model`.
      const runtime = yield* makeKiroAcpRuntime({
        kiroSettings,
        environment,
        childProcessSpawner: commandSpawner,
        cwd,
        model: resolveKiroAcpBaseModelId(modelSelection.model),
            ...
            onNone: () =>
              Effect.fail(
                new TextGenerationError({
                  operation,
                  detail: "Kiro ACP request timed out.",
                }),
              ),
