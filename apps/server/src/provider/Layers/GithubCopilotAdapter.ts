/**
 * GithubCopilotAdapter — ACP adapter for the GitHub Copilot CLI.
 *
 * Copilot ships a first-party ACP server (`copilot --acp`), so this adapter is
 * the plain ACP mapping with no vendor extension methods. Two consequences
 * shape the code below:
 *
 *   - Copilot's `session/new` carries no `SessionModelState`, so there is no
 *     in-session model switch. The model is fixed by the `--model` spawn flag
 *     (see `GithubCopilotAcpSupport`) and reported as the session's model.
 *   - Copilot has no ask-the-user extension method, so `respondToUserInput`
 *     has nothing to resolve and fails loudly if the orchestrator ever calls
 *     it for a Copilot thread.
 *
...
interface GithubCopilotSessionContext {
  ...
  /** Model the CLI was spawned with; fixed for the life of the session. */
...
export function makeGithubCopilotAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const startSession: GithubCopilotAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Copilot has no `session/set_model`, so the model is bound here, on
          // the command line, and stays fixed for the session's lifetime.
          const boundModelId = resolveGithubCopilotAcpBaseModelId(modelSelection?.model);
        ...
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const displayModel = ctx.boundModelId;
    ...
    // Copilot's ACP surface has no ask-the-user extension method, so nothing
    // ever opens a user-input request for a Copilot thread. Reaching here
    // means the orchestrator routed one to the wrong adapter.
    const respondToUserInput: GithubCopilotAdapterShape["respondToUserInput"] = (
      ...
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: `GitHub Copilot does not request user input: ${requestId}`,
        });
      });
    ...
    return {
      provider: PROVIDER,
      // The model is bound by the `--model` spawn flag; changing it needs a
      // fresh session, which is why the snapshot also sets
