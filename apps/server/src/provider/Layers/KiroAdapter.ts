/**
 * KiroAdapter — ACP adapter for Kiro CLI.
 *
 * Kiro's ACP implementation uses pre-resolved CLI credentials rather than
 * ACP `authenticate`; `KiroAcpSupport` therefore starts it without an auth
 * method and this adapter maps its standard session and permission traffic.
 *
 * @module provider/Layers/KiroAdapter
 */
import {
  ApprovalRequestId,
  type KiroSettings,
...
function parseResumeCursor(raw: unknown): { sessionId: string } | undefined {
  ...
  return value.schemaVersion === KIRO_RESUME_VERSION &&
...
export const makeKiroAdapter = (kiroSettings: KiroSettings, options?: KiroAdapterLiveOptions) =>
  Effect.gen(function* () {
    const instanceId = options?.instanceId ?? ProviderInstanceId.make("kiro");
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate a Kiro runtime identifier.",
            cause,
          }),
      ),
    );
    ...
    const closeSession = (context: KiroSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: KiroAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const model = resolveKiroAcpBaseModelId(modelSelection?.model);
          ...
          Effect.gen(function* () {
            ...
            yield* publish(
              makeAcpRequestOpenedEvent({
                ...
                detail: permission.detail ?? "Kiro requests permission.",
                args: request,
                source: "acp.jsonrpc",
            ...
            Effect.mapError(
              (cause) =>
                new EffectAcpErrors.AcpTransportError({
                  detail: "Failed to process Kiro ACP permission request.",
                  cause,
                }),
            ),
          ),
      ...
      Effect.gen(function* () {
        ...
        if (context.activeTurnId) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "sendTurn",
            issue: "Kiro is already processing a turn.",
          });
        }
    ...
    const respondToUserInput: KiroAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: `Kiro does not expose ACP user-input requests: ${requestId}`,
        });
      });
    ...
    const rollbackThread: KiroAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: `Kiro ACP sessions do not support provider-side rollback (${numTurns} turns requested).`,
        });
      });
