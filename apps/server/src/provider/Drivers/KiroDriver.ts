/**
 * KiroDriver — `ProviderDriver` for the Kiro CLI.
 *
 * Kiro ships a first-party ACP server behind `kiro-cli acp`. Credentials are
 * resolved by the CLI before its ACP process starts, then standard ACP session
 * updates flow through the shared runtime.
 *
 * Model selection is bound by Kiro's `--model` launch argument, so the model
 * remains fixed for the lifetime of an ACP session.
 *
