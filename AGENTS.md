...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs (Codex, Claude Code, Cursor, Grok, GitHub Copilot, OpenCode) and serves web, desktop, and mobile clients.
...
- **Providers.** Codex, Claude, Cursor, Grok, GitHub Copilot, and OpenCode each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
